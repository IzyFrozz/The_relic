...
func start_combat(enemy_object: Object) -> void:
	...
	if combat_ui:
		...
		print("ERROR: main.gd cannot find the CombatUI node!")


# --- MOCK ENEMY — mirrors all properties MobEnemy.gd exposes ---
class MockEnemy:
	var enemy_level: int = 1           # FIX: was missing, caused the crash
	...
	var player_is_disarmed: bool = false   # FIX: was missing, caused the crash
