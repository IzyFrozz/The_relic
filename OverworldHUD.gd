...
func _refresh() -> void:
	...
	if is_instance_valid(max_hp_label):
		max_hp_label.text = "❤️  Max HP: %d" % QuestManager.MAX_HEALTH
...
func _is_end_screen_active() -> bool:
	for name in ["LoseUI", "WinUI", "DeathScreen", "VictoryScreen", "VictoryUI"]:
		var node = get_tree().root.find_child(name, true, false)
